...
| `DP` (Departamento Pessoal) | usuário-sistema | todas (folha é company-wide) | permissões (`hasPermission`) — default-deny. ~~somente leitura~~ → desde 2026-07-08 pode receber `instr_edit`/`instr_valores` (ver §10) |
...
| Configuração | `skills_edit`, `locals_edit`, `train_edit`, `instr_view`, `instr_edit` (planejador+DP), `instr_valores` (só DP) |
| Relatórios | `reports_operacional`, `reports_financeiro`, `ai` (`reports` legado = os dois primeiros) |

**Helpers novos (2026-07-08):** `canEditInstr(u)` = `hasPermission(u,"instr_edit")` (editar cadastro,
renovar contrato); `canSeeInstrRates(u)` = `hasPermission(u,"instr_valores")` (ver/editar diárias).
admin/dev passam sempre (via `hasPermission`).
...
## 10. Revisão 2026-07-08 — DP edita cadastro + "Ver como" (APP_VERSION 47)

Motivação: **Mônica Lima (DP)** precisa editar dados cadastrais de instrutores (datas de
contrato etc.). Decisão do Matheus: DP vê **todos os relatórios**, vê e **edita o cadastro de
instrutores** (incluindo valores); não mexe em turmas/programação.

- **Furo fechado:** "Editar Dados"/"Salvar" em `instructors.js` estavam **sem gate** — qualquer
  um com acesso à página editava (contradizia o "somente leitura" do DP). Agora exigem `instr_edit`.
- **Permissões novas** (`PERMISSIONS_LIST`): `instr_edit` — Editar Cadastro (planejador, DP) e
  `instr_valores` — Ver/Editar Diárias (só DP). Diárias saíram de `canAdmin` para `canSeeInstrRates`.
- **"Renovar Contrato"** migrou de `canPlan` → `canEditInstr` (datas de contrato = cadastro).
  ⚠️ Planejadores que renovavam contrato precisam agora do checkbox `instr_edit`.
