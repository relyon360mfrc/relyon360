...
## 32. Reivindicar Programação — mudança encenada autorada pelo instrutor (2026-06-30, `APP_VERSION` 38)

Antes, aprovar uma reivindicação (`type: "reivindicacao"`) só fechava a solicitação — não lançava nada na programação (o `doApprove` só materializava tipos com `absType`). Reescrito para o modelo **"instrutor autora → planejador aprova → lança"**.

**Fluxo de dados.** O instrutor constrói um *claim* encenado no `ClaimWizard` (`communication.js`) e ele fica em `req.claim` — **nada toca os stores até a aprovação**. Formas do claim:
- `{ reason:"instrucao", action:"assumir", targetRowId, classId, className, module, moduleId, role, local, startTime, endTime, displacedInstructor* }` → na aprovação, `setSchedules` troca o `instructorId` da row alvo (substituição).
- `{ reason:"instrucao", action:"entrar", targetRowId(ref), role, local, ... }` → cria **row nova** no mesmo `classId/moduleId` (mesma forma do `savePlan`), `instructorId` = reivindicante. Unicidade OK (constraint inclui `instructorId`+`role`).
- `{ reason:"apoio", activityType, date, startTime, endTime, obs }` → `setActivities` cria atividade da Linha do Tempo. Os tipos reusam `ACTIVITY_TYPES` (`CLAIM_APOIO_TYPES`) — **sem** tocar paleta/bônus/relatórios.

**Aprovação (`materializeClaim` dentro de `ComunicacaoPage`).** Revalida `targetRowId` contra `schedules` atual (a turma pode ter mudado entre pedir e aprovar → aborta com aviso). Avisa-e-confirma (`window.confirm`) se o reivindicante tiver conflito de horário, via **`scheduleSlotConflict`** — helper global novo em `config.js` que replica a sobreposição estrita do `checkSlotConflict` do `Schedule` (que é local ao componente e não reusável de fora). Guarda `req.claimResult` (`{kind, createdRowIds|changedRowIds|activityIds, prevInstructor*}`) para o **rollback** em `_removeLinkedAbsence` (excluir a solicitação desfaz: remove rows/atividades criadas; "assumir" restaura o instrutor anterior best-effort se a row ainda existir com o reivindicante).
...
## 35. Rotina Automática de Aviso ao DP (2026-07-01 rascunho-só; passou a **enviar** em 2026-07-14 — ver §35.6)
...
### 35.2 Por que a rotina nunca enviava — só rascunhava (política até 2026-07-13 — **superada**, ver §35.6)
Enviar e-mail é ação externa irreversível; a política operacional deste projeto (e das diretrizes gerais do Claude) exige confirmação humana **no momento do envio**, não uma autorização genérica dada uma vez. Uma tarefa que dispara sozinha no fim do dia não tem como pedir "pode enviar?" a ninguém que esteja olhando. Solução (até 2026-07-13): a rotina cobria a parte mecânica (compor, formatar, endereçar) e **parava no rascunho** — o clique final de enviar continuava humano.

> **Superado em 2026-07-14** (§35.6): depois de testada e aprovada, a rotina foi autorizada a enviar sozinha. A exceção se sustenta porque o e-mail inteiro é gerado pelo app (sem conteúdo de fonte externa) — não porque a regra geral de "confirmar no momento" tenha deixado de valer nos demais casos de cowork.

### 35.3 Estado `dpNotify.status: "drafted"` (legado)
Enquanto a rotina só rascunhava, essa terceira opção evitava reprocessar o mesmo `pending` todo dia (rascunho duplicado se acumulando no Outlook). `drafted` era setado pela própria rotina via SQL direto (protocolo seguro do EXECUTE.md §11.2 — read-modify-write atômico do array inteiro de `relyon_requests`, nunca um `UPDATE` parcial). `DpNotifyPanel` mostra os dois estados juntos na mesma lista, com ações diferentes: `pending` oferece Abrir no Outlook/Copiar/Marcar enviado; `drafted` só oferece Marcar enviado. **Desde 2026-07-14 a rotina não produz mais `drafted`** (vai de `pending` direto a `sent`); o estado continua no código para itens legados que ficaram nele.

### 35.4 Fase de observação assumida (2026-07-01) — concluída em 2026-07-14
Combinado explicitamente com o Matheus: os primeiros dias dessa rotina são tratados como **experimento supervisionado**, não "automação e esquece" — ele confere o resultado (print + notificação) antes de confiar de olhos fechados. Se a extensão não estiver conectada em algum dia, a rotina só notifica a contagem de pendentes sem tentar nada visual (falha segura, não falha silenciosa). **A observação foi concluída em 2026-07-14** — a rotina graduou de rascunho-só para envio real (§35.6), mantendo a mesma falha segura.
...
- Tarefa agendada `scheduled-tasks` (fora do repo — vive em `C:\Users\mcarvalho\.claude\scheduled-tasks\aviso-dp-rascunho-outlook\SKILL.md`), prompt auto-contido (projeto Supabase, query, protocolo de composição, envio + marcação `sent` desde 2026-07-14 — ver §35.6)

### 35.6 Atualização 2026-07-14 — a rotina passou a ENVIAR
Depois de rodar em observação e ser testada/aprovada fim a fim, o Matheus autorizou a rotina a enviar de verdade (não parar mais no rascunho) e marcar `sent` no app. O que mudou no prompt da tarefa (`aviso-dp-rascunho-outlook/SKILL.md`):
- **Passo 3** compõe **e clica Enviar**, confirmando em "Itens Enviados" antes de marcar.
- **Passo 4** marca `dpNotify.status: "sent"` (`sentAt`/`sentBy`) — de preferência pelo botão "Marcar enviado" da UI (client-authoritative, §11.2), com fallback SQL + reload.
- **Guarda-corpos mantidos:** só envia destinatário/assunto/corpo vindos do `dpNotify` da query (nunca de fonte externa); sessão do Outlook indisponível (login/MFA/erro) → não envia, só relata; anti-duplicata via "Itens Enviados".

**Por que abrir exceção à regra geral de "nunca auto-enviar e-mail" (EXECUTE.md §11.1):** o risco que a regra evita é o Claude enviar conteúdo/destinatário influenciado por dados não confiáveis. Aqui o e-mail inteiro é gerado deterministicamente pelo app (`buildDpEmail`, destinatários fixos por tipo), sem superfície para conteúdo injetado — e a decisão de enviar já foi tomada pelo planejador quando aprovou a solicitação. Nenhuma mudança no código do app foi necessária (o estado `sent` já era modelado por `markDpSent`).
