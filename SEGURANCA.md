...
| **S7** | 🟡 | **Proteção de senha vazada (HIBP) desativada + política fraca** (mín. 6, sem complexidade) | advisor `auth_leaked_password_protection`; `auth.js:9` | Ativar HIBP no painel; subir mínimo p/ 8–10 ao migrar login | 🔒 **Bloqueado pelo plano** (verificado 2026-07-14: HIBP é exclusivo do plano Pro do Supabase; projeto é Free). Mínimo de senha mantido em 6 conscientemente — subir p/ 8 exige antes trocar o reset padrão `ron123` e testar o provisionamento no staging |
| **S8** | ⚪ | Funções `SECURITY DEFINER` executáveis por anon + 3 com `search_path` mutável | advisors `0011`/`0028`/`0029` | `search_path=''`; `REVOKE` das RPC de push; `enforce_*` + `rls_auto_enable` revogadas de anon/auth/PUBLIC; `current_user_*` revogadas do anon (authenticated mantido — helpers de RLS que só retornam as claims do próprio chamador) | ✅ **Fechado** (DB, 2026-06-11 + 2026-07-02). Resíduo aceito: `current_user_*` executável por authenticated (padrão Supabase p/ RLS) |
| **S9** | ⚪ | Backups com PII parados (`relyon_schedules_backup_20260502`, `_bkp_cred_20260622`, `_bkp_instr_20260622`) — RLS sem policy → não expostos, mas PII retida | advisor `rls_enabled_no_policy` | Dropar os backups redundantes (superados pelos dados vivos) | ✅ **Fechado** (DB, 2026-07-02 — 3 tabelas dropadas; minimização LGPD) |
