...
const FRACAO_TYPES = new Set(["folga_dia", "exame"]);
...
function ComunicacaoPage({ user, instructors, requests, setRequests, absences, setAbsences }) {
  ...
  const doApprove = (req, startDate, endDate) => {
    ...
    const absence = {
      ...
      startDate,
      endDate,
      startTime: req.fracaoDia && req.startTime ? req.startTime : "08:00",
      endTime:   req.fracaoDia && req.endTime   ? req.endTime   : "17:00",
      obs: req.obs || "",
    };
  ...
  // Auto-registra ausência por doença: cria ausência + solicitação aprovada + notifica instrutor
  const handleAutoSick = (instrId, instrName, obs) => {
    ...
    const absence = {
      ...
      startTime: "08:00",
      endTime: "17:00",
      obs: obs || "Doença — confirmado pelo instrutor",
    };
    ...
    createNotification({
      instructorId: instrId,
      type: "request_update",
      title: "Ausência por doença registrada",
      body: `Registrado para ${today}. Planejadores foram notificados.`,
    });
  };
    ...
    <div style={{ maxWidth: 860, margin: "0 auto" }}>
      ...
      {tab === "requisicao" && (
        <RequisicaoTab user={user} isInstr={isInstr} myRequests={myRequests}
          instructors={instructors} saveRequest={saveRequest} onAutoSick={handleAutoSick} />
      )}
...
function RequisicaoTab({ user, isInstr, myRequests, instructors, saveRequest, onAutoSick }) {
  const [selectedType, setSelectedType] = useState(null);
  const [showForm, setShowForm]         = useState(false);
  const [sickStep, setSickStep]         = useState(null); // null | "question" | "denied" | "confirmed"
  const [typeForm, setTypeForm]         = useState({
    startDate: "", endDate: "", obs: "",
    fracaoDia: false, startTime: "", endTime: "",
  });
  ...
  const openNew = () => {
    ...
    setSickStep(null);
    setTypeForm({ startDate: "", endDate: "", obs: "", fracaoDia: false, startTime: "", endTime: "" });
  };
  ...
  const handleSelectType = (rt) => {
    setSelectedType(rt);
    setSickStep(rt.id === "doenca" ? "question" : null);
  };

  const handleSickConfirm = () => {
    const instr = isInstr ? instructors.find(i => String(i.id) === String(user.instructorId)) : null;
    const instrId   = isInstr ? String(user.instructorId) : "";
    const instrName = isInstr ? (instr?.name || user.name) : user.name;
    onAutoSick(instrId, instrName, typeForm.obs);
    setSickStep("confirmed");
  };

  const handleSickDeny = () => {
    setSickStep("denied");
  };
  ...
  const handleSubmit = () => {
    ...
    if (typeForm.fracaoDia && (!typeForm.startTime || !typeForm.endTime)) {
      alert("Informe o horário de início e término da fração do dia."); return;
    }
    ...
    const req = {
      ...
      endDate:   endDate            || "",
      obs: typeForm.obs,
      fracaoDia:  typeForm.fracaoDia  || false,
      startTime:  typeForm.fracaoDia  ? typeForm.startTime : "",
      endTime:    typeForm.fracaoDia  ? typeForm.endTime   : "",
      status: "pendente",
      createdAt: new Date().toISOString(),
    };
    saveRequest(req);
      ...
      {showForm && (
        <div style={{ background: "#073d4a", borderRadius: 12, padding: 20, marginBottom: 20, border: "1px solid #154753" }}>
          {!selectedType ? (
            ...
            // ── Fluxo especial: Estou Doente ──────────────────────────────────────
            <div>
              <p style={{ color: "#ffa619", fontWeight: 700, marginBottom: 16, fontSize: 15 }}>
                {selectedType.label}
              </p>
              ...
              {sickStep === "question" && (
                <div>
                  ...
                  <div style={{ display: "flex", gap: 10 }}>
                    <Btn onClick={handleSickConfirm} label="Sim, confirmar ausência" color="#16a34a" />
                    <Btn onClick={handleSickDeny}    label="Não" color="#154753" />
                  </div>
                </div>
              )}
              ...
              {sickStep === "denied" && (
                <div>
                  <div style={{ background: "#0d4a5a", border: "1px solid #ffa619", borderRadius: 10,
                    padding: "16px 20px", marginBottom: 20 }}>
                    <p style={{ color: "#ffa619", fontWeight: 700, margin: "0 0 6px", fontSize: 14 }}>
                      Atenção
                    </p>
                  ...
                  <Btn onClick={closeForm} label="Fechar" color="#154753" />
                </div>
              )}

              {sickStep === "confirmed" && (
                <div>
                  <div style={{ background: "#14532d", border: "1px solid #4ade80", borderRadius: 10,
                    padding: "16px 20px", marginBottom: 20 }}>
                    <p style={{ color: "#4ade80", fontWeight: 700, margin: "0 0 6px", fontSize: 14 }}>
                      Ausência registrada
                    </p>
                    <p style={{ color: "#e2e8f0", fontSize: 14, margin: 0, lineHeight: 1.6 }}>
                      Sua ausência foi registrada para hoje e os Planejadores foram notificados. Melhoras!
                    </p>
                  </div>
                  <Btn onClick={closeForm} label="Fechar" color="#154753" />
                </div>
            ...
            <div>
              ...
              {selectedType.period === "single" && (
                <div>
                  ...
                  {FRACAO_TYPES.has(selectedType.id) && (
                    <div style={{ marginTop: 14 }}>
                      <label style={{ display: "flex", alignItems: "center", gap: 8,
                        color: "#e2e8f0", fontSize: 14, cursor: "pointer", userSelect: "none" }}>
                        <input type="checkbox" checked={typeForm.fracaoDia}
                          onChange={e => setTypeForm({ ...typeForm, fracaoDia: e.target.checked,
                            startTime: "", endTime: "" })}
                          style={{ width: 16, height: 16, accentColor: "#ffa619", cursor: "pointer" }} />
                        <span style={{ fontWeight: 600 }}>FRAÇÃO DO DIA</span>
                      </label>
                      {typeForm.fracaoDia && (
                        <div style={{ display: "flex", gap: 12, marginTop: 12 }}>
                          <Input label="Início" type="time" value={typeForm.startTime}
                            onChange={e => setTypeForm({ ...typeForm, startTime: e.target.value })} />
                          <Input label="Término" type="time" value={typeForm.endTime}
                            onChange={e => setTypeForm({ ...typeForm, endTime: e.target.value })} />
                        </div>
                      )}
                    </div>
                  )}
...
function RequestCard({ req, showInstructor, onApprove, onReject }) {
  ...
  const dateStr = req.startDate
    ...
    : null;
  const timeStr = req.fracaoDia && req.startTime && req.endTime
  ...
  return (
    <div style={{ background: "#073d4a", border: "1px solid #154753", borderRadius: 10,
      ...
      <div style={{ flex: 1, minWidth: 200 }}>
        ...
        {dateStr && (
          <p style={{ color: "#64748b", margin: "0 0 2px", fontSize: 12 }}>
            {dateStr}
            {timeStr && (
              <span style={{ marginLeft: 8, color: "#ffa619", fontWeight: 600 }}>{timeStr}</span>
            )}
          </p>
        )}
        {req.fracaoDia && !timeStr && (
          <p style={{ color: "#ffa619", margin: "0 0 2px", fontSize: 12 }}>Fração do dia</p>
        )}
