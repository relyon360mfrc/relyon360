...
#### 5.15.2.2 Rotina automática de aviso ao DP (2026-07-01; passou a **enviar** em 2026-07-14)

Tarefa agendada (`scheduled-tasks`, roda localmente enquanto o Claude Code está aberto) em dias úteis no fim do expediente: consulta `dpNotify.status === "pending"`; se houver navegador com a extensão Claude for Chrome conectado (perfil já logado no Outlook Web do planejador), abre o Outlook, compõe cada e-mail a partir dos campos do `dpNotify` e **envia** (confirmando em "Itens Enviados" antes de dar por enviado), depois marca `dpNotify.status: "sent"` — de preferência pelo botão "Marcar enviado" do painel (fluxo React), com fallback SQL read-modify-write atômico do array + reload (EXECUTE.md §11.2). Notifica o planejador com a contagem enviada. Se não houver navegador conectado (ou a sessão do Outlook estiver indisponível), não envia nada e só notifica a contagem de pendentes.

**Autorização de envio (2026-07-14):** até 2026-07-13 a rotina parava no rascunho (`drafted`) e nunca enviava — o clique final era humano. Depois de testada e aprovada fim a fim, o Matheus autorizou o envio de verdade. O envio autônomo é considerado seguro **só aqui** porque destinatário/assunto/corpo são 100% gerados pelo próprio app (`buildDpEmail`), nunca de fonte externa; a rotina ainda para (falha segura) se a sessão do Outlook não estiver logada, e não reenvia um aviso que já apareça em "Itens Enviados" (anti-duplicata). O estado intermediário `drafted` continua modelado no app para itens legados, mas a rotina não o produz mais (vai de `pending` direto a `sent`).
...
#### 5.16.1 Multi-base
- Usuários-sistema e instrutores têm campo `base` (`PHYSICAL_BASES` em `constants.js`: "Macaé", "Bangu"). Planejador/admin alterna a base ativa (`viewBase`) e todas as telas (Dashboard, Programação, Instrutores etc.) filtram por ela.
- **Expansão 2026-07-23 — divisão por base em TODA a estrutura (DESIGN §36.5):** GERAL = união Macaé+Bangu (`viewBase` null). Admin/dev trocam base pelos cards do Dashboard OU pelo seletor no badge da sidebar; a escolha persiste no aparelho (localStorage). Recortam pela base ativa: Locais (In Company/Online/Interno compartilhados), Usuários, Linha do Tempo, Absenteísmo (só exibição), Comunicação (solicitações pela base do solicitante), Lote Piscina (turmas + pool de instrutores). **Relatórios têm seletor próprio** (chips Geral/Macaé/Bangu, padrão Geral) — papéis DP e QSMS são company-wide e nunca são recortados. Treinamentos permanecem catálogo compartilhado. Registro sem `base` (legado) aparece em todas as bases.
